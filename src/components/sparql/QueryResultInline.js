...
export default function QueryResultInline(props) {
  ...
  useEffect(() => {
    if (!sparqlResult)
      ...
      if (sparqlResult.results.bindings.length > 0) {
        const firstRow = sparqlResult.results.bindings[0];
  ...
  }, [sparqlResult, chartType])


  if (!sparqlResult || !shouldRender)
    return <Skeleton count={10} height={20} tyle={{ borderRadius: 5 }} />

  if (chartType !== 'Table') {
    if (!chartProperties || !chartProperties.selectedValue) {
  ...
  switch (chartType) {
    case TABLE:
      renderedChart = <QueryResultGrid data={sparqlResult.results} />
      break;
    case BARCHART:
      renderedChart = <QueryResultBarChart
        data={sparqlResult.results}
