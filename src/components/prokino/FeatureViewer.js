//based on:
// https://github.com/lukasjelonek/feature-viewer/blob/master/src/feature-viewer.js
//and
// https://github.com/sacdallago/embed.protein.properties/blob/eb32b22ebc4d62f3f5aed2cab080ecb272120c95/src/components/FeatureViewer/FeatureViewer.js
// and
...
export default function FeatureViewer({sequence,featurelist}) {
    ...
    useEffect(
        () => {
            ...
            var svgElement, transit1, transit2;
            ...
            if (sequence && div.current) {
                ...
                var margin = {
                    ...
                    width = div.current.clientWidth - margin.left - margin.right - 17,
                    height = 600 - margin.top - margin.bottom;
                    

                //Init box & scaling
                d3.select('#' + div.current.id)
                ...
                function initSVG(div, options) {
                    ...
                    if (options.toolbar === true) {
                        ...
                        // var headerOptions = $(div + " .svgHeader").length ? 
                        //     d3.select(div.id + " .svgHeader") 
                        // : 
                        //     d3.select(div.id).append("div").attr("class", "svgHeader");
                        //var headerOptions = d3.select('#' + div.current.id).append("div").attr("class", "svgHeader");
                        var headerOptions = document.querySelectorAll('#' + div.current.id + " .header-position").length.length ? 
                                            d3.select('#' + div.current.id + " .svgHeader") : 
                                            d3.select('#' + div.current.id).append("div").attr("class", "svgHeader");
                
                        //if (!$(div + ' .header-position').length) 
                        if (!document.querySelectorAll('#' + div.current.id + " .header-position").length) {
                        ...
                        if (!document.querySelectorAll('#' + div.current.id + " .header-zoom").length) {
                        ...
                        var headerZoom = document.getElementById(div.current.id + ' .header-zoom') ? d3.select('#' + div.current.id + ' .header-zoom') : headerOptions;
                        if (options.bubbleHelp === true) {
                            //if (!$(div.id + ' .header-help').length) 
                            if (!document.querySelectorAll('#' + div.current.id + " .header-help").length) {
                                ...
                                buttonHelp
                                    ...
                                    .style("color", "#000")
                                    .html("<span class='state'>Show help</span>");
                                ...
                                //todo: fix
                                 //   $(function () {
                                    ...
                                    // let headerHelp = document.querySelectorAll('#' + div.current.id + " .header-help");
                                    // headerHelp.on('hide.bs.popover', function () {
                                    //     $(this).find(".state").text("Show");
                                    // });
                                    // headerHelp.on('show.bs.popover', function () {
                ...
                svg =  d3.select(div.current).append("svg")
                ...
                svgElement = div.current.getElementsByTagName("svg")[0];
                svgContainer = svg
                            .append("g")
                ...
                d3.helper.tooltip = function (object) {
                    var tooltipDiv;
                    var selectedRect;
                    var bodyNode = d3.select(div.current).node();
                    ...
                    function tooltip(selection) {
                        ...
                        selection.on('mouseover.tooltip', function (pD, pI) {
                            ...
                            if (rightside) {
                                tooltipDiv = d3.select(div.current)
                                    .append('div')
                            ...
                            .on('click', function (pD, pI) {
                                ...
                                // Append tooltip
                                selectedRect = d3.select(div.current)
                                    .append('div')
                                ...
                                selectedRect.style({
                                    left: xRect + 'px',
                                    top: document.querySelector(div.current.id + " .svgHeader") ? 60 + 'px' : 10 + 'px',
                                    'background-color': 'rgba(0, 0, 0, 0.2)',
                                    width: widthRect + 'px',
                                ...
                                if (CustomEvent) {
                                    var event = new CustomEvent(events.FEATURE_SELECTED_EVENT, {
                                        detail: {
                                ...
                                if (self.trigger) self.trigger(events.FEATURE_SELECTED_EVENT, {
                                    start: object.type === "path" ? pD[0].x : object.type === "line" ? elemHover.x : pD.x,
                                    end: object.type === "path" ? pD[1].x : object.type === "line" ? elemHover.y : pD.y,
                                    id: object.type === "path" ? pD[0].id : object.type === "line" ? elemHover.id : pD.id,
                                    description: object.type === "path" ? pD[0].description : object.type === "line" ? elemHover.description : pD.description
                ...
                let onFeatureSelected = function (listener) {
                    svgElement.addEventListener(events.FEATURE_SELECTED_EVENT, listener);
                };
                let onFeatureDeselected = function (listener) {
                    svgElement.addEventListener(events.FEATURE_DESELECTED_EVENT, listener);
                };

                let onZoom = function (listener) {
                    svgElement.addEventListener(events.ZOOM_EVENT, listener);
                };
                ...
                function forcePropagation(item) {
                    item.on('mousedown', function () {
                        //todo: fix
                        // let brush_elm = svg.select(".brush").node();
                ...
                var fillSVG = {
                    ...
                    group: function (object, position, level) {
                        var rectHeight = 8;
                        var rectShift = 10;
                        var rects = svgContainer.append("g")
                            .attr("class", "multipleRects")
                ...
                let showFilteredFeature = function (className, color, baseUrl) {
                ...
                let hideFilteredFeature = function (className) {
                ...
                let resetZoom = function (start, end) {
                    resetAll();
                }

                function brushend() {
                    d3.select(div.current).selectAll('div.selectedRect').remove();
                    if (Object.keys(featureSelected).length !== 0 && featureSelected.constructor === Object) {
                        ...
                        featureSelected = {};
                        if (CustomEvent) {
                            var event = new CustomEvent(events.FEATURE_DESELECTED_EVENT, {
                        ...
                        if (self.trigger) self.trigger(events.FEATURE_DESELECTED_EVENT, { info: "feature-deselected" });
                    }
                    ...
                    if (!brush.empty() && extentLength > zoomMax) {
                        ...
                        d3.select('#' + div.current.id + " .zoomUnit").text(zoomScale.toString());

                        if (SVGOptions.showSequence && !(intLength) && seq && d3.select(div.current).selectAll(".AA").empty()) {
                            current_extend = {
                                length: extentLength,
                                start: start,
                                end: end
                            ...
                            seqShift = start;
                            d3.select(div.current).selectAll(".sequenceLine").remove();
                        ...
                        if (self.trigger) self.trigger(events.ZOOM_EVENT, {
                            start: start,
                            end: end,
                            zoom: zoomScale
                        });

                        d3.select(div.current).selectAll(".brush").call(brush.clear());
                    } else {
                        d3.select(div.current).selectAll(".brush").call(brush.clear());
                    }
                }
                ...
                svgElement.addEventListener("mousemove", 
                    function(evt) {
                        var absoluteMousePos = SVGOptions.brushActive ? 
                        clientPoint(d3.select(".background").node(),evt) 
                        : 
                        clientPoint(d3.select("svg").node,evt);        
                    var pos = Math.round(scalingPosition(absoluteMousePos[0]));
                    if (!options.positionWithoutLetter) {
                        pos += sequence[pos-1] || "";
                    }
                    document.getElementById("zoomPosition").innerText= pos;
                   });
                ...
                function updateWindow() {
                    //todo: check width
                    width = div.current.offsetWidth - margin.left - margin.right - 17;
                    d3.select('#' + div.current.id + " svg")
                        .attr("width", width + margin.left + margin.right);
                    d3.select('#' + div.current.id + " #clip>rect").attr("width", width);
                    if (SVGOptions.brushActive) {
                        d3.select('#' + div.current.id + " .background").attr("width", width);
                    }
                    d3.select('#' + div.current.id).selectAll(".brush").call(brush.clear());
                ...
                function resetAll() {
                    ...
                    //reset scale
                    document.getElementsByClassName('zoomUnit')[0].innerText="1";
                    ...
                    // Fire Event
                    if (CustomEvent) {
                        svgElement.dispatchEvent(new CustomEvent(events.ZOOM_EVENT,
                ...
                let addRectSelection = function (svgId) {
                    ...
                    // Append tooltip
                    var selectedRect = d3.select(div.current)
                        .append('div')
                        .attr('class', 'selectedRect');
                ...
                function addFeature(object) {
                    ...
                    updateSVGHeight(Yposition);
                    if (SVGOptions.brushActive) {
                        let brushRect = svgContainer.selectAll(".brush rect");
                        brushRect.attr('height', Yposition + 50);
                    }
                ...
                if (featurelist)
                    featurelist.map(f => addFeature(f));
        ...
        [sequence, div.current]);

    return (
        //     <Head>
        //     <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js" integrity="sha256-4+XzXVhsDmqanXGHaHvgh1gMQKX40OUvDEBTu8JcmNs=" crossorigin="anonymous"></script>
        //   </Head>
        <div style={{position: "relative", padding: "0px", zIndex: 2, opacity: 100}}>
        <div id={"fv1"} ref={div} >
            {/* <svg
                width={400}
                height={200}
